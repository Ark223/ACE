...
namespace Ace
{
    ...
    /// </summary>
    using Aliases = Dictionary<Node, IReadOnlyList<Card>>;
    ...
    internal sealed partial class MCTS
    {
        ...
        /// <summary>
        /// Finds adjacent cards of the same suit surrounding the selected card.
        /// </summary>
        /// <param name="node">Node whose action defines the lookup card.</param>
        private IReadOnlyList<Card> GetAliases(in Node node)
        {
            var group = new List<Card>(13);
            Card best = node.Action;

            // Build rank lookup for all legal moves
            var table = new Dictionary<int, Card>();
            foreach (Card action in this._deal.Moves)
            {
                // Skip cards containing different suit
                if (action.Suit != best.Suit) continue;
                table.Add(action.Rank, action);
            }

            // Walk down through cards of the same suit
            int low = best.Rank - 1, high = best.Rank + 1;
            while (table.TryGetValue(low--, out Card lower))
            {
                group.Add(lower);
            }

            // Do the same for higher ranks in the same suit
            while (table.TryGetValue(high++, out Card upper))
            {
                group.Add(upper);
            }
            return group;
        ...
        private bool Scan(double exploration)
        {
            ...
            {
                // Cache aliases linked to selected node
                var aliases = this.GetAliases(best.node);
                this._aliases[best.node] = aliases;
        ...
        private void Update(in Node node, in Outcome outcome)
        {
            ...
            // Check if the node has registered equivalent actions
            if (!this._aliases.TryGetValue(node, out var aliases))
            {
                return;
            }
